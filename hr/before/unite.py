...
for link in vacancies:
    ...
    # Check from my handwritten library if position and company name are written in latin letters, signifying that vacancy is in english (at list details are in english)
    result1 = any(elem in list(position) for elem in letters)
    result2 = any(elem in list(company) for elem in letters)
    ...
    # Dates
    try:
        published = str(details).split("<strong>თარიღები:</strong>")
    ...
    # Location
    try:
        location = str(details).split("<strong>მდებარეობა:</strong>")
    ...
    # Job Type
    try:
        jtype = str(details).split("<strong> დასაქმების ფორმა:</strong>")
    ...
    # Salary + Bonuses
    try:
        salary = str(details).split("<strong> ხელფასი:</strong>")
    ...
    # Experience
    try:
        experience = str(details).split("<strong> გამოცდილება:</strong>")
    ...
    # Education
    try:
        education = str(details).split("<strong> განათლება:</strong>")
    ...
    # Languages
    try:
        languages = str(details).split("<strong> ენები:</strong>")
    ...
    # Driver's License
    try:
        dLicense = str(details).split("<strong> მართვის მოწმობა:</strong>")
    ...
    # Phone Number
    try:
        pNumber = str(details).split("<strong> ტელეფონი:</strong>")
    ...
    # Address
    try:
        address = str(details).split("<strong> მისამართი:</strong>")
