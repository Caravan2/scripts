...
for i in range(1, 10):
    i = i * 9
    try:
        url = f'http://repatarmenia.org/en/engage/careers/LatestPostsFilter?jsonData={"{"}%22Count%22:{i},%22LoadMoreCount%22:%229%22,%22SortBy%22:0{"}"}'
        ...
        for n in range(0, 9):
            ...
            if email == []:
                ...
                check = userdb.find_one({"email" : email[0]})
                if check is None:
                    new_user_info = {
                        "email" : email[0],
                    ...
                    user_object_id = userdb.find_one({"email" : email[0]})
